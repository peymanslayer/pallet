...
export class KilometerDetailsService {
    constructor(
        ...
        private kilometerDetailRepository: typeof KilometerDetails,
    ){}
    async create(carNumber: string ,driverId:number ,kilometer: number){
        ...
        const kilometerDetails = await this.kilometerDetailRepository.findOne({where : {driverId}})
        if(!kilometerDetails) {
            return {
              status: 200 , 
              data : [] ,
              message : "جزئیات کیلومتر مورد نظر یافت نشد"
            }
        }

        kilometerDetails.kilometer += kilometer;
    ...
    async getKilometerDetailsByFilter(
        ...
        if (carNumbers && carNumbers.length > 0) {
          filter.carNumber = { [Op.in]: carNumbers };
        }
        ...
        const kilometerDetails = await this.kilometerDetailRepository.findAll({
          where: filter,
        });
      ...
      async deleteKilometerDetails(id: number){
        const kilometerDetails = await this.kilometerDetailRepository.findByPk(id)
        if(!kilometerDetails){
            return {
                status: 201,
                data: [] ,
        ...
        await this.kilometerDetailRepository.destroy({
            where : {id}
        })
